LOVELY_INTEGRITY = '41161bdade016db8670f472d6b906287dc7c24066ef67a94ba3c3c0bbbd93ab9'
...
SMODS.enh_cache:write(self, nil)
    ...
    if self.ability and old_center and old_center.config.bonus then
        self.ability.bonus = (self.ability.bonus or 0) - (old_center.config.bonus or 0)
    end
...
function Card:set_cost()
    ...
    self.cost = math.max(1, math.floor((self.base_cost + self.extra_cost + 0.5)*(100-G.GAME.discount_percent)/100))
        ...
        if not G.GAME.modifiers.cry_misprint_min and not G.GAME.entr_shop_price_modifier then self.cost = math.floor(self.cost) end end
