LOVELY_INTEGRITY = 'a65638826a308fb9f79a4820f9bc2a547a2651c1a00bcc9bd795592c9eaaeeb0'
...
function SMODS.find_card(key, count_debuffed)
    ...
    for _, area in ipairs(SMODS.get_card_areas('jokers')) do
        if area.cards then
            for _, v in pairs(area.cards or {}) do
...
function SMODS.get_enhancements(card, extra_only)
    -- Improved nil check with logging
    if not card then 
        print("Error: card is nil in get_enhancements. Called with extra_only:", extra_only)
        return {} 
    end
    ...
    if not SMODS.enh_cache:read(card, extra_only) then
        ...
        if card.config and card.config.center and card.config.center.key ~= "c_base" then
            ...
            print("Warning: card.config or card.config.center is nil for card:", card)
        end
...
function SMODS.get_card_areas(_type, _context)
    ...
    if _type == 'jokers' then
        ...
        if G.GAME.modifiers.cry_beta then t = {G.jokers} end
        return t
    end
    if _type == 'individual' then
        local t = {
            { object = G.GAME.selected_back, scored_card = ((G.deck or {}).cards or {})[1] or G.deck },
        }
        if G.GAME.blind then t[#t+1] = { object = G.GAME.blind, scored_card = ((G.GAME.blind or {}).children or {}).animatedSprite or nil } end
...
function SMODS.eval_individual(individual, context)
    ...
    if not individual or not individual.object then return ret, post_trig end
