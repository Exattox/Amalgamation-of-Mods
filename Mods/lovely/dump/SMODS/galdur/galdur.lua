LOVELY_INTEGRITY = 'd946523e963f841a49cc1b236350a3552050a80db975bf09b032ffe58416c3cd'
...
function generate_deck_card_areas()
    ...
    for i=1, 18 do
        Galdur.run_setup.deck_select_areas[i] = CardArea(G.ROOM.T.w,G.ROOM.T.h, G.CARD_W*.8, G.CARD_H*.8, 
...
function populate_deck_card_areas(page)
    local count = 1 + (page - 1) * 18
    for i=1, 18 do
        if count > #G.P_CENTER_POOLS.Back then return end
        local card_number = Galdur.config.reduce and 1 or 10
        for index = 1, card_number do
            local card = Card(Galdur.run_setup.deck_select_areas[i].T.x,Galdur.run_setup.deck_select_areas[i].T.y, G.CARD_W*.8, G.CARD_H*.8, G.P_CENTER_POOLS.Back[count], G.P_CENTER_POOLS.Back[count],
...
function create_deck_page_cycle()
    local options = {}
    local cycle
    if #G.P_CENTER_POOLS.Back > 18 then
        local total_pages = math.ceil(#G.P_CENTER_POOLS.Back / 18)
